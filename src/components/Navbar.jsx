    ...
    <nav className="bg-gray-900 text-white sticky top-0 z-50">
      <div className="px-4 py-3 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-4">
          ...
          <Link to="/orders" className="text-sm text-gray-300 hover:text-orange-400">
            📦 Orders
          </Link>
      ...
      {menuOpen && (
        <div className="md:hidden bg-gray-800 px-4 py-3 flex flex-col gap-3 border-t border-gray-700">
          ...
          <Link to="/" onClick={() => setMenuOpen(false)}
            className="text-sm text-gray-300 py-2 border-b border-gray-700">🏠 Home</Link>
          <Link to="/wishlist" onClick={() => setMenuOpen(false)}
            className="text-sm text-gray-300 py-2 border-b border-gray-700 flex justify-between">
            <span>❤️ Wishlist</span>
            {wishlist.length > 0 && <span className="bg-red-500 text-white text-xs px-2 py-0.5 rounded-full">{wishlist.length}</span>}
          </Link>
          <Link to="/orders" onClick={() => setMenuOpen(false)}
            className="text-sm text-gray-300 py-2 border-b border-gray-700">📦 Orders</Link>
          ...
          <button onClick={handleLogout}
            className="bg-red-500 text-white py-2 rounded-xl text-sm font-bold">
            Logout
          </button>
        </div>
      )}
